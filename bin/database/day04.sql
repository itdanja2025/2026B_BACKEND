# ctrl+shift+enter ( 파일내 위에서부터 아래로 하나씩 실행 )
# DB 생성
...
SELECT mid , sum( bamount * bprice ) 총구매금액 FROM buy GROUP BY mid;  -- 2) mid 기준으로 총 구매금액( 수량 * 가격 )
SELECT count(*) , mid FROM buy GROUP BY mid;                            -- 3) mid(회원)별 구매 횟수 
...
# [*] 다중정렬 이란? 첫번째 정렬 후 첫번째 정렬 필드 기준으로 중복이 존재한경우 중복끼리 2차정렬 
